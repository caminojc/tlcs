/* TLC codec tuning parameters — SMPL-matched CELP + SCOREQ-optimized TCX
 * Runtime-overridable via environment variables (no recompile needed). */
#ifndef TLCS_TUNE_H
#define TLCS_TUNE_H
...
/* ── TCX mode params (SCOREQ-optimized on 20-file corpus) ── */
...
/* ── CELP mode params (SMPL-matched) ── */
static inline float tune_celp_harm_str(void)    { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_HARM_STR",    0.6400f); return v; }
static inline float tune_celp_pf_num(void)      { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_PF_NUM",      0.9700f); return v; }
static inline float tune_celp_pf_den(void)      { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_PF_DEN",      0.9900f); return v; }
static inline float tune_celp_pf_tilt(void)     { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_PF_TILT",     0.1000f); return v; }
static inline float tune_celp_nf_voiced(void)   { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_NF_VOICED",   0.3500f); return v; }
static inline float tune_celp_nf_unvoiced(void) { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_NF_UNVOICED", 0.8000f); return v; }
static inline float tune_celp_gamma1(void)      { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_GAMMA1",      0.9400f); return v; }
static inline float tune_celp_gamma2(void)      { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_GAMMA2",      0.6000f); return v; }
static inline float tune_celp_pitch_sharp(void) { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_PITCH_SHARP", 0.8500f); return v; }
static inline float tune_celp_hp_shelf(void)    { static float v=-999; if(v<-900) v=tune_get_("TUNE_CELP_HP_SHELF",    0.2500f); return v; }
...
#define TUNE_CELP_PITCH_SHARP tune_celp_pitch_sharp()
#define TUNE_CELP_HP_SHELF    tune_celp_hp_shelf()
